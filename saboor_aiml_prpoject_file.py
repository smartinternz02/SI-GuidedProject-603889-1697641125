# -*- coding: utf-8 -*-
"""saboor AIML PRPOJECT FILE

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1swCq1t0fzYjRkTYVXIWwWKxQfwYGEioY
...
data = pd.read_csv('Lumpy skin data.csv')
data.head()
...
# Assuming you have a DataFrame 'lumpy_skin_data' with features and labels
# Replace the following with your actual feature columns and label column

features = data.drop('cld', 'dtr', 'frs',
             'pet', 'pre', 'tmn', 'tmp', 'tmx', 'vap',
             'wet', 'elevation', 'dominant_land_cover', axis=1)
labels = data['lumpy']
...
svm_model = SVC(kernel='linear', C=1.0)  # You can choose different parameters for your specific problem
...
columns_to_drop = ['cld', 'dtr', 'frs',
                   'pet', 'pre', 'tmn', 'tmp', 'tmx', 'vap',
                   'wet', 'elevation', 'dominant_land_cover']
...
knn_model = KNeighborsClassifier(n_neighbors=5)  # You can choose a different number of neighbors
...
# Plot Confusion Matrix in red
plt.imshow(conf_matrix, interpolation='nearest', cmap=plt.cm.Reds)  # Change cmap to Reds
...
# Initialize the Naive Bayes (NB) model
nb_model = GaussianNB()
...
plt.imshow(conf_matrix, interpolation='nearest', cmap=plt.cm.Greens)  # Change cmap to Greens
...
# Reshape features for a simple CNN example (adjust according to your data)
features_reshaped = np.array(features).reshape(len(features), 1, len(features.columns), 1)
...
cnn_model.add(Conv2D(32, kernel_size=(3, 3), activation='relu', input_shape=(1, len(features.columns), 1)))
cnn_model.add(MaxPooling2D(pool_size=(2, 2)))
cnn_model.add(Flatten())
cnn_model.add(Dense(64, activation='relu'))
cnn_model.add(Dense(len(set(labels)), activation='softmax'))  # Adjust output layer based on the number of classes
...
plt.imshow(conf_matrix, interpolation='nearest', cmap=plt.cm.Yellows)  # Change cmap to Yellows
plt.title('Confusion Matrix')
plt.colorbar()

classes = set(np.argmax(y_test, axis=1))
tick_marks = range(len(classes))
plt.xticks(tick_marks, classes)
plt.yticks(tick_marks, classes)
...
# Plot ROC Curve
fpr, tpr, thresholds = roc_curve(np.argmax(y_test, axis=1), predictions)
roc_auc = auc(fpr, tpr)
...
# Reshape features for a simple CNN example (adjust according to your data)
features_reshaped = np.array(features).reshape(len(features), len(features.columns), 1)
...
# Initialize the CNN model
cnn_model = Sequential()
cnn_model.add(Conv1D(32, kernel_size=3, activation='relu', input_shape=(len(features.columns), 1)))
...
# Plot Confusion Matrix in yellow
# Plot Confusion Matrix in yellow
plt.imshow(conf_matrix, interpolation='nearest', cmap=plt.cm.YlOrBr)  # Change cmap to YlOrBr or another yellowish colormap
plt.title('Confusion Matrix for CNN')
plt.colorbar()
  # Change cmap to Yellows
plt.title('Confusion Matrix')
plt.colorbar()
  # Change cmap to Yellows
plt.title('Confusion Matrix for CNN')
...
# Plot Confusion Matrix in brown
plt.imshow(conf_matrix, interpolation='nearest', cmap=plt.cm.YlOrBr)  # Change cmap to YlOrBr or another brownish colormap
...
# Vary the hyperparameter (e.g., max depth of the tree)
...
# Vary the hyperparameter (C value)
...
plt.semilogx(C_values, test_accuracies, label='Testing Accuracy', marker='o')
plt.xlabel('C value (Inverse of regularization strength)')
plt.ylabel('Accuracy')
plt.title('Training and Testing Accuracy vs. C value for SVM')
plt.legend()
plt.grid(True)
plt.show()
...
# Vary the hyperparameter (k value)
k_values = range(1, 21)
train_accuracies = []
test_accuracies = []
...
plt.ylabel('Accuracy')
plt.title('Training and Testing Accuracy vs. k value for KNN')
plt.legend()
plt.grid(True)
plt.show()
...
plt.ylabel('Accuracy')
plt.title('Training and Testing Accuracy for Naive Bayes')
plt.show()
...
cnn_model = keras.Sequential([
    layers.Conv2D(32, kernel_size=(3, 3), activation='relu', input_shape=(1, X_train.shape[2], 1)),
...
# Increase the input size or adjust the architecture
...
# Assuming X_train and X_test have shapes like (19842, features_count)
# Reshape data for CNN
X_train = X_train.reshape(X_train.shape[0], 1, X_train.shape[1], 1)
X_test = X_test.reshape(X_test.shape[0], 1, X_test.shape[1], 1)
...
# Vary the hyperparameter (number of trees in the forest)
...
for n_estimators_value in n_estimators_values:
    rf_model = RandomForestClassifier(n_estimators=n_estimators_value, random_state=42)
    rf_model.fit(X_train, y_train)
...
plt.ylabel('Accuracy')
plt.title('Training and Testing Accuracy vs. Number of Trees for Random Forest')
plt.legend()
plt.grid(True)
plt.show()